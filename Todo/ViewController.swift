...
class ViewController: UIViewController {
  ...
  override func viewWillAppear(_ animated: Bool) {
    ...
    let result = LocalStorage.sharedInstance.getTodo()
    todos = result
...
extension ViewController: UITableViewDelegate {
  ...
  func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
     ...
     var result = LocalStorage.sharedInstance.getTodo()
     result.remove(at: (indexPath as NSIndexPath).row)
     LocalStorage.sharedInstance.saveTodo(todo: result)
     let postResult = LocalStorage.sharedInstance.getTodo()
     todos = postResult
  ...
  func tableView(_ tableView: UITableView, moveRowAt sourceIndexPath: IndexPath, to destinationIndexPath: IndexPath) {
    ...
    var result = LocalStorage.sharedInstance.getTodo()
    let todo = result.remove(at:  (sourceIndexPath as NSIndexPath).row)

    //let todo = todos.remove(at: (sourceIndexPath as NSIndexPath).row)
    result.insert(todo, at: (destinationIndexPath as NSIndexPath).row)
    LocalStorage.sharedInstance.saveTodo(todo: result)
